...
/*
  ...
  This is used in plugin initilization. We check for
  plugin specific names aws-kms-key.<id>.<version>
  to find out what keys/key versions are already there.
*/
static vector<string> read_dir()
{
  vector<string> v;
  DBUG_ENTER("read_dir");
#ifdef _WIN32
  WIN32_FIND_DATA ffd;
  HANDLE h= FindFirstFile("*", &ffd);
  ...
  do
  {
    /* handle file possibly updates caches*/
...
static int plugin_init(void *p)
{
  ...
  vector<string> filenames= read_dir();
  for (size_t i= 0; i < filenames.size(); i++)
  {
    KEY_INFO info;
    if (extract_id_and_version(filenames[i].c_str(), &info.key_id, &info.key_version) == 0)
    {
  ...
  else // (ver == 0)
  {
    ...
    if (!master_key_id[0])
    {
      my_printf_error(ER_UNKNOWN_ERROR,
        "Can't generate key %u, because aws_key_management_master_key_id parameter is not set",
        MYF(ME_JUST_WARNING), key_id);
      DBUG_RETURN(ENCRYPTION_KEY_VERSION_INVALID);
    }
